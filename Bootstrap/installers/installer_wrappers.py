...
# Pip wrapper template
PIP_WRAPPER_TEMPLATE = '''#!/bin/bash
# JoyBox pip3 wrapper - uses venv pip
# Generated by JoyBox Bootstrap
exec "{venv_pip}" "$@"
'''
...
class Wrappers(installer.Installer):
    def __init__(
        ...
        # Get venv pip path
        if python_venv_dir:
            self.venv_pip = os.path.expandvars(os.path.join(python_venv_dir, "bin", "pip3"))
        else:
    ...
    def install(self):
        ...
        if self.venv_exists:
            ...
            if self.connection.write_file(temp_path, pip_wrapper_content):
                self.connection.move_file_or_directory(temp_path, pip_wrapper_path)
                self.connection.change_permission(pip_wrapper_path, "755")
                installed_wrappers.append("pip3")
