...
class GitHooks(installer.Installer):
    def __init__(
        ...
        # JoyBox path
        self.joybox_root = util.get_repo_root(self.config, expand = True)
    ...
    def install(self):
        ...
        # Start install
        util.log_info("Configuring git hooks for the JoyBox repo")

        # The hooks directory must exist in the repo
        if not self.connection.does_file_or_directory_exist(self.hooks_dir):
            util.log_error(f"Hooks directory not found: {self.hooks_dir}")
            return False
        ...
        if not self.flags.pretend_run and not self.is_installed():
            util.log_error("Failed to set core.hooksPath")
            return False

        # All done
        util.log_info(f"git hooks active (core.hooksPath = {self.hooks_path})")
        return True

    def uninstall(self):

        # Start uninstall
        util.log_info("Unconfiguring git hooks for the JoyBox repo")
        ...
        # All done
        util.log_info("git hooks deactivated")
        return True
