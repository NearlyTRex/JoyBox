...
# Back up the server's databases and app data to the Storage Box
python3 bootstrap.py -a backup -t remote_ubuntu -s 0

# Restore one service (see Backup and Restore for the required flags)
python3 bootstrap.py -a restore -t remote_ubuntu -s 0 \
    --components wordpress --backup-id latest --confirm restore

# Show the pinned container image for every docker component
python3 bootstrap.py -t remote_ubuntu --list-images
...
| Flag | Meaning |
|------|---------|
| `-a`, `--action` | `setup`, `teardown`, `status`, `backup`, or `restore` |
...
| `--confirm` | Type `restore` to confirm a destructive restore |
| `--purge-data` | Delete container volumes and the app directory during teardown. **Destroys databases and app data**; without it both are kept |
