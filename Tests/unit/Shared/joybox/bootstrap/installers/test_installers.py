...
def test_published_ports_bind_loopback(app_name, installers_dir):
    ...
    path = os.path.join(installers_dir, f"installer_{app_name}.py")
...
def test_every_compose_template_is_covered_by_the_port_test(installers_dir):
...
def nginx_template_sources(installers_dir):
...
def test_tls_vhosts_include_shared_params(installers_dir):
    ...
    offenders = []
    for filename, source in nginx_template_sources(installers_dir):
...
def test_tls_vhosts_reference_the_apex_cert_path(installers_dir):
