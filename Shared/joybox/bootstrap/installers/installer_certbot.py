...
class Certbot(installer.Installer):
    ...
    def install_selfsigned_cert(self):
        ...
        # Generated by the certbot manager, which the account's sudo grant covers.
        logger.log_info("Generating a self-signed certificate on the target")
        self.connection.run_checked(
            [self.cert_manager_tool, "selfsign"] + self.fully_qualified_domains, sudo = True)
        return True

    def write_cert_pair(self, cert_contents, key_contents):

        # Staged in a directory only the account can enter, then installed by the
        # certbot manager, which the account's sudo grant covers
        staging_dir = f"/tmp/joybox-cert-{uuid.uuid4().hex[:8]}"
        if not self.connection.make_directory(staging_dir):
            logger.log_error("Could not stage the certificate on the target")
            return False
        try:
            self.connection.change_permission(staging_dir, "700")
            staged = {}
            for filename, contents in [("fullchain.pem", cert_contents), ("privkey.pem", key_contents)]:
                staged[filename] = f"{staging_dir}/{filename}"
                if not self.connection.write_file(staged[filename], contents):
                    logger.log_error(f"Could not stage {filename} on the target")
                    return False
            self.connection.run_checked([
                self.cert_manager_tool, "install_pair", self.domain_name,
                staged["fullchain.pem"], staged["privkey.pem"]], sudo = True)
        finally:
            self.connection.remove_file_or_directory(staging_dir)
        logger.log_info(f"Installed certificate to {self.get_cert_dir()}")
        return True
